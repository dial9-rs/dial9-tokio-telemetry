...
enum ControlCommand {
    /// Drain the collector and flush the writer, then ack.
...
impl TelemetryHandle {
    ...
    pub fn disable(&self) {
        ...
        // Ask the flush thread to do one flush cycle and wait for ack.
        let (ack_tx, ack_rx) = std::sync::mpsc::sync_channel(0);
        // If the flush thread is already gone, the send fails — that's fine,
        // it means everything was already finalized.
        if self.control_tx.send(ControlCommand::Flush(ack_tx)).is_ok() {
            let _ = ack_rx.recv();
        }
    }
            ...
            std::thread::Builder::new()
                ...
                .spawn(move || {
                    ...
                    loop {
                        ...
                        // these are sent during `disable` and during shutdown.
                        // the normal flush flow happens below.
                        match control_rx.recv_timeout(Duration::from_millis(5)) {
                            // sent when the `.disable()` is called on the handle
                            Ok(ControlCommand::Flush(ack)) => {
                                // fall through a do a normal flush
                                ack_tx = Some(ack);
                            }
