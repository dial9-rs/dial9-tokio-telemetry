...
fn js_decodes_all_field_types() {
    ...
    enc.register_schema_for::<AllTypes>("AllTypes", vec![
        FieldDef { name: "a_u64".into(), field_type: FieldType::U64 },
        FieldDef { name: "b_i64".into(), field_type: FieldType::I64 },
        FieldDef { name: "c_f64".into(), field_type: FieldType::F64 },
    ...
    let pool_id = enc.intern_string("hello");
    ...
    assert!((vals["c_f64"].as_f64().unwrap() - 3.14).abs() < 1e-10);
...
fn js_decodes_multiple_events() {
    let mut enc = Encoder::new();
    enc.register_schema_for::<Tick>("Tick", vec![
        FieldDef { name: "ts".into(), field_type: FieldType::Varint },
    ]);
    for i in 0..5u64 {
