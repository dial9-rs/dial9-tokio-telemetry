...
fn full_round_trip() {
    ...
    enc.register_schema_for::<PollStart>("PollStart", vec![
        FieldDef { name: "timestamp_ns".into(), field_type: FieldType::U64 },
        FieldDef { name: "worker".into(), field_type: FieldType::U64 },
        FieldDef { name: "task_id".into(), field_type: FieldType::U64 },
    ]);
    enc.register_schema_for::<CpuSample>("CpuSample", vec![
        FieldDef { name: "timestamp_ns".into(), field_type: FieldType::U64 },
        FieldDef { name: "thread_name".into(), field_type: FieldType::PooledString },
        FieldDef { name: "frames".into(), field_type: FieldType::StackFrames },
    ]);

    let thread_id = enc.intern_string("worker-0");
    ...
    if let DecodedFrame::Event { values, .. } = &decoded[3] {
        assert_eq!(*values, vec![FieldValue::U64(1_000_000), FieldValue::U64(0), FieldValue::U64(42)]);
    } else {
...
fn round_trip_all_field_types() {
    let mut enc = Encoder::new();
    enc.register_schema_for::<AllTypes>("AllTypes", vec![
        FieldDef { name: "a".into(), field_type: FieldType::U64 },
        FieldDef { name: "b".into(), field_type: FieldType::I64 },
        FieldDef { name: "c".into(), field_type: FieldType::F64 },
    ...
    let pool_id = enc.intern_string("test");
    let values = vec![
    ...
    if let DecodedFrame::Event { values: decoded_values, .. } = event {
