//! Tests validating SPEC.md edge cases and format limits.

use dial9_trace_format::codec::{self, Frame, PoolEntry, SymbolEntry, HEADER_SIZE, MAGIC, VERSION};
use dial9_trace_format::decoder::{DecodedFrame, Decoder};
use dial9_trace_format::encoder::Encoder;
use dial9_trace_format::schema::{FieldDef, SchemaEntry};
use dial9_trace_format::types::{FieldType, FieldValue};

// --- Header edge cases ---

#[test]
fn header_is_exactly_5_bytes() {
    let mut buf = Vec::new();
    codec::encode_header(&mut buf);
    assert_eq!(buf.len(), HEADER_SIZE);
    assert_eq!(&buf[..4], &MAGIC);
    assert_eq!(buf[4], VERSION);
}

#[test]
fn empty_stream_after_header() {
    let data = Encoder::new().finish();
    assert_eq!(data.len(), 5);
    let mut dec = Decoder::new(&data).unwrap();
    assert!(dec.next_frame().is_none());
}

// --- Schema edge cases (codec-level, not encoder) ---

#[test]
fn schema_max_type_id() {
    // Test at codec level: max type_id round-trips
    let entry = SchemaEntry {
        type_id: u16::MAX,
        name: "Max".into(),
        fields: vec![FieldDef { name: "v".into(), field_type: FieldType::U64 }],
    };
    let mut buf = Vec::new();
    codec::encode_schema(&entry, &mut buf);
    let types = vec![FieldType::U64];
    let lookup = |id: u16| -> Option<&[FieldType]> {
        if id == u16::MAX { Some(&types) } else { None }
    };
    // Decode schema
    let (frame, _) = codec::decode_frame(&buf, &|_| None).unwrap();
    assert_eq!(frame, Frame::Schema(entry));

    // Encode and decode an event with that type_id
    let mut event_buf = Vec::new();
    codec::encode_event(u16::MAX, &[FieldValue::U64(1)], &mut event_buf);
    let (frame, _) = codec::decode_frame(&event_buf, &lookup).unwrap();
    assert!(matches!(frame, Frame::Event { type_id, .. } if type_id == u16::MAX));
}

#[test]
fn schema_empty_name() {
    let entry = SchemaEntry { type_id: 0, name: "".into(), fields: vec![] };
    let mut buf = Vec::new();
    codec::encode_schema(&entry, &mut buf);
    let (frame, _) = codec::decode_frame(&buf, &|_| None).unwrap();
    assert_eq!(frame, Frame::Schema(entry));
}

#[test]
fn schema_many_fields() {
    let fields: Vec<FieldDef> = (0..256)
        .map(|i| FieldDef { name: format!("f{i}"), field_type: FieldType::Varint })
        .collect();
    let entry = SchemaEntry { type_id: 1, name: "Wide".into(), fields: fields.clone() };
    let mut buf = Vec::new();
    codec::encode_schema(&entry, &mut buf);
    let (frame, _) = codec::decode_frame(&buf, &|_| None).unwrap();
    assert_eq!(frame, Frame::Schema(entry));
}

// --- Field type edge cases ---

#[test]
fn u64_boundary_values() {
    for val in [0u64, 1, u64::MAX] {
        let v = FieldValue::U64(val);
        let mut buf = Vec::new();
        v.encode(&mut buf);
        let (decoded, _) = FieldValue::decode(FieldType::U64, &buf, 0).unwrap();
        assert_eq!(decoded, v);
    }
}

#[test]
fn i64_boundary_values() {
    for val in [i64::MIN, -1, 0, 1, i64::MAX] {
        let v = FieldValue::I64(val);
        let mut buf = Vec::new();
        v.encode(&mut buf);
        let (decoded, _) = FieldValue::decode(FieldType::I64, &buf, 0).unwrap();
        assert_eq!(decoded, v);
    }
}

#[test]
fn f64_special_values() {
    for val in [0.0, -0.0, f64::INFINITY, f64::NEG_INFINITY] {
        let v = FieldValue::F64(val);
        let mut buf = Vec::new();
        v.encode(&mut buf);
        let (decoded, _) = FieldValue::decode(FieldType::F64, &buf, 0).unwrap();
        assert_eq!(decoded, v);
    }
    let v = FieldValue::F64(f64::NAN);
    let mut buf = Vec::new();
    v.encode(&mut buf);
    let (decoded, _) = FieldValue::decode(FieldType::F64, &buf, 0).unwrap();
    if let FieldValue::F64(d) = decoded {
        assert!(d.is_nan());
    }
}

#[test]
fn empty_string_field() {
    let v = FieldValue::String(vec![]);
    let mut buf = Vec::new();
    v.encode(&mut buf);
    assert_eq!(buf.len(), 4);
    let (decoded, consumed) = FieldValue::decode(FieldType::String, &buf, 0).unwrap();
    assert_eq!(consumed, 4);
    assert_eq!(decoded, v);
}

#[test]
fn empty_bytes_field() {
    let v = FieldValue::Bytes(vec![]);
    let mut buf = Vec::new();
    v.encode(&mut buf);
    let (decoded, _) = FieldValue::decode(FieldType::Bytes, &buf, 0).unwrap();
    assert_eq!(decoded, v);
}

#[test]
fn empty_u64_array() {
    let v = FieldValue::U64Array(vec![]);
    let mut buf = Vec::new();
    v.encode(&mut buf);
    assert_eq!(buf.len(), 4);
    let (decoded, _) = FieldValue::decode(FieldType::U64Array, &buf, 0).unwrap();
    assert_eq!(decoded, v);
}

#[test]
fn empty_stack_frames() {
    let v = FieldValue::StackFrames(vec![]);
    let mut buf = Vec::new();
    v.encode(&mut buf);
    assert_eq!(buf.len(), 4);
    let (decoded, _) = FieldValue::decode(FieldType::StackFrames, &buf, 0).unwrap();
    assert_eq!(decoded, v);
}

#[test]
fn varint_zero() {
    let v = FieldValue::Varint(0);
    let mut buf = Vec::new();
    v.encode(&mut buf);
    assert_eq!(buf.len(), 1);
    assert_eq!(buf[0], 0x00);
    let (decoded, consumed) = FieldValue::decode(FieldType::Varint, &buf, 0).unwrap();
    assert_eq!(consumed, 1);
    assert_eq!(decoded, v);
}

#[test]
fn varint_max() {
    let v = FieldValue::Varint(u64::MAX);
    let mut buf = Vec::new();
    v.encode(&mut buf);
    assert_eq!(buf.len(), 10);
    let (decoded, consumed) = FieldValue::decode(FieldType::Varint, &buf, 0).unwrap();
    assert_eq!(consumed, 10);
    assert_eq!(decoded, v);
}

#[test]
fn varint_leb128_boundary_values() {
    let cases = [(127u64, 1), (128, 2), (16383, 2), (16384, 3)];
    for (val, expected_bytes) in cases {
        let v = FieldValue::Varint(val);
        let mut buf = Vec::new();
        v.encode(&mut buf);
        assert_eq!(buf.len(), expected_bytes, "Varint({val}) should be {expected_bytes} bytes");
        let (decoded, _) = FieldValue::decode(FieldType::Varint, &buf, 0).unwrap();
        assert_eq!(decoded, v);
    }
}

// --- StackFrames delta encoding ---

#[test]
fn stack_frames_single_address() {
    let v = FieldValue::StackFrames(vec![0x7fff_ffff_ffff]);
    let mut buf = Vec::new();
    v.encode(&mut buf);
    let (decoded, _) = FieldValue::decode(FieldType::StackFrames, &buf, 0).unwrap();
    assert_eq!(decoded, v);
}

#[test]
fn stack_frames_identical_addresses() {
    let addrs = vec![0x1000u64; 5];
    let v = FieldValue::StackFrames(addrs);
    let mut buf = Vec::new();
    v.encode(&mut buf);
    assert!(buf.len() <= 10);
    let (decoded, _) = FieldValue::decode(FieldType::StackFrames, &buf, 0).unwrap();
    assert_eq!(decoded, v);
}

#[test]
fn stack_frames_descending_addresses() {
    let addrs = vec![0x5555_5555_5000u64, 0x5555_5555_4000, 0x5555_5555_3000];
    let v = FieldValue::StackFrames(addrs);
    let mut buf = Vec::new();
    v.encode(&mut buf);
    let (decoded, _) = FieldValue::decode(FieldType::StackFrames, &buf, 0).unwrap();
    assert_eq!(decoded, v);
}

// --- String pool edge cases ---

#[test]
fn string_pool_empty_string() {
    let entries = vec![PoolEntry { pool_id: 0, data: vec![] }];
    let mut buf = Vec::new();
    codec::encode_string_pool(&entries, &mut buf);
    let (frame, _) = codec::decode_frame(&buf, &|_| None).unwrap();
    assert_eq!(frame, Frame::StringPool(entries));
}

#[test]
fn string_pool_many_entries() {
    let entries: Vec<PoolEntry> = (0..100)
        .map(|i| PoolEntry { pool_id: i, data: format!("str_{i}").into_bytes() })
        .collect();
    let mut buf = Vec::new();
    codec::encode_string_pool(&entries, &mut buf);
    let (frame, _) = codec::decode_frame(&buf, &|_| None).unwrap();
    assert_eq!(frame, Frame::StringPool(entries));
}

// --- Symbol table edge cases ---

#[test]
fn symbol_table_max_address() {
    let entries = vec![SymbolEntry { base_addr: u64::MAX, size: u32::MAX, symbol_id: u32::MAX }];
    let mut buf = Vec::new();
    codec::encode_symbol_table(&entries, &mut buf);
    let (frame, _) = codec::decode_frame(&buf, &|_| None).unwrap();
    assert_eq!(frame, Frame::SymbolTable(entries));
}

// --- Multi-frame ordering ---

struct Ev0; struct Ev1; struct Ev2; struct Ev3; struct Ev4;

#[test]
fn multiple_schemas_then_events() {
    let markers: [fn(&mut Encoder) -> u16; 5] = [
        |e| e.register_schema_for::<Ev0>("Ev0", vec![FieldDef { name: "v".into(), field_type: FieldType::Varint }]),
        |e| e.register_schema_for::<Ev1>("Ev1", vec![FieldDef { name: "v".into(), field_type: FieldType::Varint }]),
        |e| e.register_schema_for::<Ev2>("Ev2", vec![FieldDef { name: "v".into(), field_type: FieldType::Varint }]),
        |e| e.register_schema_for::<Ev3>("Ev3", vec![FieldDef { name: "v".into(), field_type: FieldType::Varint }]),
        |e| e.register_schema_for::<Ev4>("Ev4", vec![FieldDef { name: "v".into(), field_type: FieldType::Varint }]),
    ];
    let writers: [fn(&mut Encoder, u64); 5] = [
        |e, v| e.write_event_for::<Ev0>(&[FieldValue::Varint(v)]),
        |e, v| e.write_event_for::<Ev1>(&[FieldValue::Varint(v)]),
        |e, v| e.write_event_for::<Ev2>(&[FieldValue::Varint(v)]),
        |e, v| e.write_event_for::<Ev3>(&[FieldValue::Varint(v)]),
        |e, v| e.write_event_for::<Ev4>(&[FieldValue::Varint(v)]),
    ];
    let mut enc = Encoder::new();
    for reg in &markers { reg(&mut enc); }
    for (i, write) in writers.iter().enumerate() { write(&mut enc, i as u64); }
    let data = enc.finish();
    let mut dec = Decoder::new(&data).unwrap();
    let frames = dec.decode_all();
    assert_eq!(frames.len(), 10);
    for f in &frames[..5] { assert!(matches!(f, DecodedFrame::Schema(_))); }
    for f in &frames[5..] { assert!(matches!(f, DecodedFrame::Event { .. })); }
}

struct PoolEv;

#[test]
fn interleaved_pool_and_events() {
    let mut enc = Encoder::new();
    enc.register_schema_for::<PoolEv>("Ev", vec![
        FieldDef { name: "s".into(), field_type: FieldType::PooledString },
    ]);
    let id0 = enc.intern_string("first");
    enc.write_event_for::<PoolEv>(&[FieldValue::PooledString(id0.pool_id())]);
    let id1 = enc.intern_string("second");
    enc.write_event_for::<PoolEv>(&[FieldValue::PooledString(id1.pool_id())]);
    let data = enc.finish();

    let mut dec = Decoder::new(&data).unwrap();
    let frames = dec.decode_all();
    assert_eq!(frames.len(), 5);
    assert_eq!(dec.string_pool.get(&id0.pool_id()), Some(&"first".to_string()));
    assert_eq!(dec.string_pool.get(&id1.pool_id()), Some(&"second".to_string()));
}

// --- Field type tag exhaustiveness ---

#[test]
fn all_field_type_tags_valid() {
    for tag in 0u8..=10 {
        assert!(FieldType::from_tag(tag).is_some(), "tag {tag} should be valid");
    }
}

#[test]
fn field_type_tag_11_invalid() {
    assert!(FieldType::from_tag(11).is_none());
}

#[test]
fn field_type_tag_255_invalid() {
    assert!(FieldType::from_tag(255).is_none());
}

// --- Truncated data handling ---

#[test]
fn truncated_header() {
    assert!(Decoder::new(&[0x54, 0x52, 0x43]).is_none());
}

#[test]
fn truncated_event_frame() {
    let types = vec![FieldType::U64];
    let data = [0x02, 0x01];
    let result = codec::decode_frame(&data, &|_| Some(&types));
    assert!(result.is_none());
}

#[test]
fn truncated_schema_frame() {
    let data = [0x01, 0x00, 0x00];
    let result = codec::decode_frame(&data, &|_| None);
    assert!(result.is_none());
}
