//! Kitchen-sink round-trip tests: encode via derive macro, decode via
//! TraceEvent::decode() into typed Ref structs, verify every field type,
//! edge cases, multi-event streams, and interleaved string pools.

use dial9_trace_format::decoder::{DecodedFrameRef, Decoder};
use dial9_trace_format::encoder::Encoder;
use dial9_trace_format::types::FieldValueRef;
use dial9_trace_format::{InternedString, StackFrames, TraceEvent};

// ── Structs covering every supported field type ─────────────────────────

#[derive(TraceEvent)]
struct KitchenSink {
    a_u8: u8,
    b_u16: u16,
    c_u32: u32,
    d_u64: u64,
    e_i64: i64,
    f_f64: f64,
    g_bool: bool,
    h_string: String,
    i_bytes: Vec<u8>,
    j_interned: InternedString,
    k_frames: StackFrames,
    l_map: Vec<(String, String)>,
}

#[derive(TraceEvent)]
struct Empty {}

#[derive(TraceEvent)]
struct SingleVarint {
    v: u64,
}

#[derive(TraceEvent)]
struct TwoStrings {
    a: String,
    b: String,
}

#[derive(TraceEvent)]
struct MultiPool {
    x: InternedString,
    y: InternedString,
    z: InternedString,
}

// ── Helpers ─────────────────────────────────────────────────────────────

/// Decode all ref frames, returning only events as (type_id, values) pairs.
fn decode_events(data: &[u8]) -> (Decoder<'_>, Vec<(u16, Vec<FieldValueRef<'_>>)>) {
    let mut dec = Decoder::new(data).unwrap();
    let mut events = Vec::new();
    for frame in dec.decode_all_ref() {
        if let DecodedFrameRef::Event { type_id, values } = frame {
            events.push((type_id, values));
        }
    }
    (dec, events)
}

// ── Tests ───────────────────────────────────────────────────────────────

#[test]
fn kitchen_sink_all_field_types() {
    let mut enc = Encoder::new();
    let interned = enc.intern_string("hello_pool");

    let ev = KitchenSink {
        a_u8: 255,
        b_u16: 65535,
        c_u32: 0xDEAD_BEEF,
        d_u64: u64::MAX,
        e_i64: i64::MIN,
        f_f64: std::f64::consts::PI,
        g_bool: true,
        h_string: "hello world 🌍".to_string(),
        i_bytes: vec![0x00, 0xFF, 0xAB, 0xCD],
        j_interned: interned,
        k_frames: StackFrames(vec![0x5555_5555_1234, 0x5555_5555_0a00, 0x5555_5555_0800]),
        l_map: vec![
            ("key1".to_string(), "val1".to_string()),
            ("key2".to_string(), "val2".to_string()),
        ],
    };
    enc.write(&ev);
    let data = enc.finish();

    let mut dec = Decoder::new(&data).unwrap();
    let frames = dec.decode_all_ref();
    let event = frames.iter().find_map(|f| match f {
        DecodedFrameRef::Event { values, .. } => Some(values),
        _ => None,
    }).unwrap();

    let decoded = KitchenSink::decode(event).unwrap();

    assert_eq!(decoded.a_u8, 255);
    assert_eq!(decoded.b_u16, 65535);
    assert_eq!(decoded.c_u32, 0xDEAD_BEEF);
    assert_eq!(decoded.d_u64, u64::MAX);
    assert_eq!(decoded.e_i64, i64::MIN);
    assert!((decoded.f_f64 - std::f64::consts::PI).abs() < f64::EPSILON);
    assert_eq!(decoded.g_bool, true);
    assert_eq!(decoded.h_string, "hello world 🌍");
    assert_eq!(decoded.i_bytes, &[0x00, 0xFF, 0xAB, 0xCD]);
    assert_eq!(decoded.j_interned, interned);
    assert_eq!(decoded.k_frames.iter().collect::<Vec<_>>(),
               vec![0x5555_5555_1234, 0x5555_5555_0a00, 0x5555_5555_0800]);
    let map_pairs: Vec<_> = decoded.l_map.iter().collect();
    assert_eq!(map_pairs, vec![
        (&b"key1"[..], &b"val1"[..]),
        (&b"key2"[..], &b"val2"[..]),
    ]);

    // Verify pool was resolved
    assert_eq!(dec.string_pool.get(&interned.0), Some(&"hello_pool".to_string()));
}

#[test]
fn kitchen_sink_zero_and_empty_values() {
    let mut enc = Encoder::new();
    let interned = enc.intern_string("");

    let ev = KitchenSink {
        a_u8: 0,
        b_u16: 0,
        c_u32: 0,
        d_u64: 0,
        e_i64: 0,
        f_f64: 0.0,
        g_bool: false,
        h_string: String::new(),
        i_bytes: vec![],
        j_interned: interned,
        k_frames: StackFrames(vec![]),
        l_map: vec![],
    };
    enc.write(&ev);
    let data = enc.finish();

    let mut dec = Decoder::new(&data).unwrap();
    let frames = dec.decode_all_ref();
    let event = frames.iter().find_map(|f| match f {
        DecodedFrameRef::Event { values, .. } => Some(values),
        _ => None,
    }).unwrap();

    let decoded = KitchenSink::decode(event).unwrap();

    assert_eq!(decoded.a_u8, 0);
    assert_eq!(decoded.b_u16, 0);
    assert_eq!(decoded.c_u32, 0);
    assert_eq!(decoded.d_u64, 0);
    assert_eq!(decoded.e_i64, 0);
    assert_eq!(decoded.f_f64, 0.0);
    assert_eq!(decoded.g_bool, false);
    assert_eq!(decoded.h_string, "");
    assert!(decoded.i_bytes.is_empty());
    assert_eq!(decoded.k_frames.count(), 0);
    assert_eq!(decoded.l_map.count(), 0);
}

#[test]
fn empty_struct_round_trip() {
    let mut enc = Encoder::new();
    enc.write(&Empty {});
    enc.write(&Empty {});
    let data = enc.finish();

    let (_, events) = decode_events(&data);
    assert_eq!(events.len(), 2);
    for (_, values) in &events {
        let decoded = Empty::decode(values).unwrap();
        let _ = decoded; // just verify it decodes
    }
}

#[test]
fn multi_event_stream_preserves_order() {
    let mut enc = Encoder::new();
    for i in 0..100u64 {
        enc.write(&SingleVarint { v: i });
    }
    let data = enc.finish();

    let (_, events) = decode_events(&data);
    assert_eq!(events.len(), 100);
    for (i, (_, values)) in events.iter().enumerate() {
        let decoded = SingleVarint::decode(values).unwrap();
        assert_eq!(decoded.v, i as u64);
    }
}

#[test]
fn interleaved_event_types() {
    let mut enc = Encoder::new();
    // Alternate between two event types
    for i in 0..20u64 {
        enc.write(&SingleVarint { v: i });
        enc.write(&TwoStrings {
            a: format!("a{i}"),
            b: format!("b{i}"),
        });
    }
    let data = enc.finish();

    let mut dec = Decoder::new(&data).unwrap();
    let frames = dec.decode_all_ref();
    let events: Vec<_> = frames.iter().filter_map(|f| match f {
        DecodedFrameRef::Event { type_id, values } => Some((*type_id, values)),
        _ => None,
    }).collect();

    assert_eq!(events.len(), 40);
    // Events alternate: SingleVarint, TwoStrings, SingleVarint, TwoStrings...
    let varint_tid = events[0].0;
    let strings_tid = events[1].0;
    assert_ne!(varint_tid, strings_tid);

    for i in 0..20 {
        let (tid, vals) = &events[i * 2];
        assert_eq!(*tid, varint_tid);
        let sv = SingleVarint::decode(vals).unwrap();
        assert_eq!(sv.v, i as u64);

        let (tid, vals) = &events[i * 2 + 1];
        assert_eq!(*tid, strings_tid);
        let ts = TwoStrings::decode(vals).unwrap();
        assert_eq!(ts.a, format!("a{i}"));
        assert_eq!(ts.b, format!("b{i}"));
    }
}

#[test]
fn interleaved_string_pools() {
    let mut enc = Encoder::new();
    // Intern strings interleaved with events that reference them
    let id_a = enc.intern_string("alpha");
    enc.write(&MultiPool { x: id_a, y: id_a, z: id_a });

    let id_b = enc.intern_string("beta");
    enc.write(&MultiPool { x: id_a, y: id_b, z: id_a });

    let id_c = enc.intern_string("gamma");
    enc.write(&MultiPool { x: id_c, y: id_b, z: id_a });

    let data = enc.finish();

    let mut dec = Decoder::new(&data).unwrap();
    let frames = dec.decode_all_ref();
    let events: Vec<_> = frames.iter().filter_map(|f| match f {
        DecodedFrameRef::Event { values, .. } => Some(values),
        _ => None,
    }).collect();

    assert_eq!(events.len(), 3);

    let e0 = MultiPool::decode(&events[0]).unwrap();
    assert_eq!(e0.x, id_a);
    assert_eq!(e0.y, id_a);
    assert_eq!(e0.z, id_a);

    let e1 = MultiPool::decode(&events[1]).unwrap();
    assert_eq!(e1.x, id_a);
    assert_eq!(e1.y, id_b);
    assert_eq!(e1.z, id_a);

    let e2 = MultiPool::decode(&events[2]).unwrap();
    assert_eq!(e2.x, id_c);
    assert_eq!(e2.y, id_b);
    assert_eq!(e2.z, id_a);

    // All pool entries resolved
    assert_eq!(dec.string_pool.get(&id_a.0).unwrap(), "alpha");
    assert_eq!(dec.string_pool.get(&id_b.0).unwrap(), "beta");
    assert_eq!(dec.string_pool.get(&id_c.0).unwrap(), "gamma");
}

#[test]
fn string_pool_deduplication() {
    let mut enc = Encoder::new();
    let id1 = enc.intern_string("same");
    let id2 = enc.intern_string("same");
    let id3 = enc.intern_string("different");
    assert_eq!(id1, id2);
    assert_ne!(id1, id3);

    enc.write(&MultiPool { x: id1, y: id2, z: id3 });
    let data = enc.finish();

    let mut dec = Decoder::new(&data).unwrap();
    let frames = dec.decode_all_ref();
    let event = frames.iter().find_map(|f| match f {
        DecodedFrameRef::Event { values, .. } => Some(values),
        _ => None,
    }).unwrap();

    let decoded = MultiPool::decode(event).unwrap();
    assert_eq!(decoded.x, decoded.y); // same pool ID
    assert_ne!(decoded.x, decoded.z);
}

#[test]
fn stack_frames_edge_cases() {
    #[derive(TraceEvent)]
    struct FrameEvent {
        frames: StackFrames,
    }

    let cases: Vec<Vec<u64>> = vec![
        vec![],                                          // empty
        vec![0],                                         // single zero
        vec![u64::MAX],                                  // max address
        vec![0x1000; 100],                               // 100 identical
        vec![0x5555_5555_5000, 0x5555_5555_4000, 0x5555_5555_3000], // descending
        vec![1, 2, 3, 4, 5],                             // ascending small
        vec![0, u64::MAX, 0, u64::MAX],                  // alternating extremes
    ];

    for addrs in &cases {
        let mut enc = Encoder::new();
        enc.write(&FrameEvent { frames: StackFrames(addrs.clone()) });
        let data = enc.finish();

        let mut dec = Decoder::new(&data).unwrap();
        let frames = dec.decode_all_ref();
        let event = frames.iter().find_map(|f| match f {
            DecodedFrameRef::Event { values, .. } => Some(values),
            _ => None,
        }).unwrap();

        let decoded = FrameEvent::decode(event).unwrap();
        let decoded_addrs: Vec<u64> = decoded.frames.iter().collect();
        assert_eq!(&decoded_addrs, addrs, "failed for input {addrs:?}");
    }
}

#[test]
fn f64_special_values_round_trip() {
    #[derive(TraceEvent)]
    struct Floats {
        a: f64,
        b: f64,
        c: f64,
        d: f64,
    }

    let mut enc = Encoder::new();
    enc.write(&Floats {
        a: f64::INFINITY,
        b: f64::NEG_INFINITY,
        c: -0.0,
        d: f64::MIN_POSITIVE,
    });
    let data = enc.finish();

    let mut dec = Decoder::new(&data).unwrap();
    let frames = dec.decode_all_ref();
    let event = frames.iter().find_map(|f| match f {
        DecodedFrameRef::Event { values, .. } => Some(values),
        _ => None,
    }).unwrap();

    let decoded = Floats::decode(event).unwrap();
    assert_eq!(decoded.a, f64::INFINITY);
    assert_eq!(decoded.b, f64::NEG_INFINITY);
    assert!(decoded.c.is_sign_negative() && decoded.c == 0.0);
    assert_eq!(decoded.d, f64::MIN_POSITIVE);
}

#[test]
fn f64_nan_round_trip() {
    #[derive(TraceEvent)]
    struct NanEvent { v: f64 }

    let mut enc = Encoder::new();
    enc.write(&NanEvent { v: f64::NAN });
    let data = enc.finish();

    let mut dec = Decoder::new(&data).unwrap();
    let frames = dec.decode_all_ref();
    let event = frames.iter().find_map(|f| match f {
        DecodedFrameRef::Event { values, .. } => Some(values),
        _ => None,
    }).unwrap();

    let decoded = NanEvent::decode(event).unwrap();
    assert!(decoded.v.is_nan());
}

#[test]
fn unicode_string_round_trip() {
    let mut enc = Encoder::new();
    enc.write(&TwoStrings {
        a: "日本語テスト 🎌".to_string(),
        b: "émojis: 🦀🔥💯 and ñ".to_string(),
    });
    let data = enc.finish();

    let mut dec = Decoder::new(&data).unwrap();
    let frames = dec.decode_all_ref();
    let event = frames.iter().find_map(|f| match f {
        DecodedFrameRef::Event { values, .. } => Some(values),
        _ => None,
    }).unwrap();

    let decoded = TwoStrings::decode(event).unwrap();
    assert_eq!(decoded.a, "日本語テスト 🎌");
    assert_eq!(decoded.b, "émojis: 🦀🔥💯 and ñ");
}

#[test]
fn large_bytes_field() {
    #[derive(TraceEvent)]
    struct BlobEvent { data: Vec<u8> }

    let big = vec![0xABu8; 64 * 1024]; // 64KB
    let mut enc = Encoder::new();
    enc.write(&BlobEvent { data: big.clone() });
    let data = enc.finish();

    let mut dec = Decoder::new(&data).unwrap();
    let frames = dec.decode_all_ref();
    let event = frames.iter().find_map(|f| match f {
        DecodedFrameRef::Event { values, .. } => Some(values),
        _ => None,
    }).unwrap();

    let decoded = BlobEvent::decode(event).unwrap();
    assert_eq!(decoded.data.len(), 64 * 1024);
    assert!(decoded.data.iter().all(|&b| b == 0xAB));
}

#[test]
fn varint_boundary_values() {
    #[derive(TraceEvent)]
    struct Boundaries {
        a: u8,
        b: u16,
        c: u32,
        d: u64,
    }

    let mut enc = Encoder::new();
    // LEB128 boundaries: 127/128, 16383/16384
    enc.write(&Boundaries { a: 127, b: 128, c: 16383, d: 16384 });
    enc.write(&Boundaries { a: u8::MAX, b: u16::MAX, c: u32::MAX, d: u64::MAX });
    enc.write(&Boundaries { a: 0, b: 0, c: 0, d: 0 });
    let data = enc.finish();

    let (_, events) = decode_events(&data);
    assert_eq!(events.len(), 3);

    let d0 = Boundaries::decode(&events[0].1).unwrap();
    assert_eq!((d0.a, d0.b, d0.c, d0.d), (127, 128, 16383, 16384));

    let d1 = Boundaries::decode(&events[1].1).unwrap();
    assert_eq!((d1.a, d1.b, d1.c, d1.d), (u8::MAX, u16::MAX, u32::MAX, u64::MAX));

    let d2 = Boundaries::decode(&events[2].1).unwrap();
    assert_eq!((d2.a, d2.b, d2.c, d2.d), (0, 0, 0, 0));
}

#[test]
fn decode_wrong_field_count_returns_none() {
    // Manually construct a FieldValueRef slice that's too short
    let vals: Vec<FieldValueRef<'_>> = vec![FieldValueRef::Varint(1)];
    // TwoStrings expects 2 fields, giving it 1 should fail
    assert!(TwoStrings::decode(&vals).is_none());
    // Empty slice
    assert!(SingleVarint::decode(&[]).is_none());
}

#[test]
fn string_map_round_trip() {
    #[derive(TraceEvent)]
    struct MapEvent {
        tags: Vec<(String, String)>,
    }

    let mut enc = Encoder::new();
    enc.write(&MapEvent {
        tags: vec![
            ("env".into(), "prod".into()),
            ("region".into(), "us-east-1".into()),
            ("service".into(), "api-gateway".into()),
        ],
    });
    // Empty map
    enc.write(&MapEvent { tags: vec![] });
    // Unicode keys/values
    enc.write(&MapEvent {
        tags: vec![("名前".into(), "テスト".into())],
    });
    let data = enc.finish();

    let (_, events) = decode_events(&data);
    assert_eq!(events.len(), 3);

    let d0 = MapEvent::decode(&events[0].1).unwrap();
    let pairs0: Vec<_> = d0.tags.iter().collect();
    assert_eq!(pairs0, vec![
        (&b"env"[..], &b"prod"[..]),
        (b"region".as_slice(), b"us-east-1".as_slice()),
        (b"service".as_slice(), b"api-gateway".as_slice()),
    ]);

    let d1 = MapEvent::decode(&events[1].1).unwrap();
    assert_eq!(d1.tags.count(), 0);

    let d2 = MapEvent::decode(&events[2].1).unwrap();
    let pairs2: Vec<_> = d2.tags.iter().collect();
    assert_eq!(pairs2, vec![("名前".as_bytes(), "テスト".as_bytes())]);
}

#[test]
fn many_events_many_types_stress() {
    #[derive(TraceEvent)]
    struct TypeA { ts: u64, val: u32 }
    #[derive(TraceEvent)]
    struct TypeB { name: String, flag: bool }
    #[derive(TraceEvent)]
    struct TypeC { x: f64, y: f64 }

    let mut enc = Encoder::new();
    let n = 500;
    for i in 0..n {
        match i % 3 {
            0 => enc.write(&TypeA { ts: i as u64 * 1000, val: i as u32 }),
            1 => enc.write(&TypeB { name: format!("ev{i}"), flag: i % 2 == 0 }),
            2 => enc.write(&TypeC { x: i as f64 * 0.1, y: -(i as f64) }),
            _ => unreachable!(),
        }
    }
    let data = enc.finish();

    let mut dec = Decoder::new(&data).unwrap();
    let frames = dec.decode_all_ref();
    let events: Vec<_> = frames.iter().filter_map(|f| match f {
        DecodedFrameRef::Event { type_id, values } => Some((*type_id, values)),
        _ => None,
    }).collect();
    assert_eq!(events.len(), n);

    // Resolve type_ids by name
    let a_name = TypeA::event_name();
    let b_name = TypeB::event_name();
    let c_name = TypeC::event_name();

    for (idx, (tid, vals)) in events.iter().enumerate() {
        let schema_name = &dec.registry().get(*tid).unwrap().name;
        match idx % 3 {
            0 => {
                assert_eq!(schema_name, a_name);
                let d = TypeA::decode(vals).unwrap();
                assert_eq!(d.ts, idx as u64 * 1000);
                assert_eq!(d.val, idx as u32);
            }
            1 => {
                assert_eq!(schema_name, b_name);
                let d = TypeB::decode(vals).unwrap();
                assert_eq!(d.name, format!("ev{idx}"));
                assert_eq!(d.flag, idx % 2 == 0);
            }
            2 => {
                assert_eq!(schema_name, c_name);
                let d = TypeC::decode(vals).unwrap();
                assert!((d.x - idx as f64 * 0.1).abs() < f64::EPSILON);
                assert!((d.y - -(idx as f64)).abs() < f64::EPSILON);
            }
            _ => unreachable!(),
        }
    }
}

#[test]
fn with_capacity_encoder() {
    let mut enc = Encoder::with_capacity(64);
    enc.write(&SingleVarint { v: 42 });
    let data = enc.finish();

    let (_, events) = decode_events(&data);
    assert_eq!(events.len(), 1);
    let decoded = SingleVarint::decode(&events[0].1).unwrap();
    assert_eq!(decoded.v, 42);
}
