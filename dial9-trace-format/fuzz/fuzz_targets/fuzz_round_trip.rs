...
fuzz_target!(|data: &[u8]| {
    ...
    enc.register_schema_for::<Ev>("FuzzEvent", vec![
        FieldDef { name: "a".into(), field_type: FieldType::Varint },
        FieldDef { name: "b".into(), field_type: FieldType::Varint },
    ]);
    ...
    let event_count = frames.iter().filter(|f| matches!(f, DecodedFrame::Event { .. })).count();
