...
fn captures_lock_acquisition_stack() {
    ...
    for sample in &samples {
        for &addr in &sample.callchain {
            let info = dbg!(resolve_symbol(dbg!(addr)));
...
fn captures_sleep_stack() {
    ...
    for sample in &samples {
        for &addr in &sample.callchain {
            ...
            if let Some(name) = dbg!(info).name {
    ...
    assert!(
        resolved_names
            .iter()
            .any(|n| n.contains("sleep") || n.contains("main") || n.contains("sched_triggers"))
            || !samples.is_empty(),
        "expected symbols from the test binary in sleep stack or at least some samples. \
         Got {} samples with {} resolved names. \
         Note: frame pointer unwinding may produce shallow stacks in test binaries.",
        samples.len(),
        resolved_names.len()
    );
}
